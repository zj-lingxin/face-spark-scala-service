...
class BusinessCircleImpl extends IBusinessCircle {
  @Resource @BeanProperty var sparkBaseFactory: ISparkBaseFactory = null
   ...
   * @return 返回结果集
   */
  override def searchFriendFrame(loginAccount: String): DataFrame = ???
   ...
   * @return 返回结果集
   */
  override def searchTwoFriendFrame(loginAccount: String): DataFrame = ???

  /**
   * 更新数据源 定时任务调取
   * @return 返回执行状态
   */
  override def updateDBSources: Boolean = {
  ...
  override def searchPersonRelation(userIds: util.List[String], loginAccount: String): util.List[PersonRelation] = {
    ...
    //我的所有的一度好友
    val myFriends = sqlContext.sql(s"select distinct loginAccount from ofRoster where username = '${loginAccount}' and username <> loginAccount ").map(_ (0).toString).collect()
